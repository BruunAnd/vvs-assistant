...
namespace VVSAssistant.Database
{
    ...
    public class BulkInsertProvider : ProviderBase<SqlCeConnection, SqlCeTransaction>
    {
        public BulkInsertProvider()
        {
        ...
        {
            using (var dbConnection = new AssistantContext().Database.Connection)
            {
                dbConnection.Open();
        ...
        public override void Run<T>(IEnumerable<T> entities, SqlCeTransaction transaction, EntityFramework.BulkInsert.Extensions.BulkInsertOptions options)
        {
            Options = options;
            Run(entities, (SqlCeConnection)transaction.Connection, transaction);
        }
